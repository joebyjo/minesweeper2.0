...
void CellMatrix::set_mines() {
    ...
    for(int i = 0; i < num_mines; i++) {
        ...
        if(!(check_if_member(location))) {
            mine_locations.push_back(location);
        } else {
            i--;
        };
...
void CellMatrix::set_game_window(RenderWindow* game_window) {
    this->game_window = game_window;
};
...
RenderWindow* CellMatrix::get_game_window() {
