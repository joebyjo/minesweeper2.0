...
Number::Number(int x, int y) : Cell(x, y) {
    ...
    number_cell->setScale((CELL_SIZE - padding) / float(number_texture.getSize().x),
                          (CELL_SIZE - padding) / float(number_texture.getSize().y));

    // centering the sprite within the cell
    number_cell->setPosition(location[0] * CELL_SIZE + (padding / 2), location[1] * CELL_SIZE + (padding / 2));
